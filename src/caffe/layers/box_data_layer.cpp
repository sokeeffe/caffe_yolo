...
void BoxDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    char file_name[200];
    sprintf(file_name, "models/Region_Detector/Dec2018/lmdb_input_%d_%d_%d_%d.csv", 
    ...
    if (this->output_labels_) {
      ...
      sprintf(file_name, "models/Region_Detector/Dec2018/modified_labels_%d_%d_%d_%d.csv", 
              iter_, this->transformed_data_.shape(1),
              this->transformed_data_.shape(2),this->transformed_data_.shape(3));
