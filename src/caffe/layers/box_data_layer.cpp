...
template<typename Dtype>
void fill_truth_detection(Dtype* top_label, int label_offset, int batch_size, int flip, float dx, float dy, float sx, float sy)
{
    Dtype* top_label_temp;
    top_label_temp = (Dtype*)calloc(150*batch_size, sizeof(Dtype));
    memcpy(top_label_temp, top_label, 150*batch_size*sizeof(Dtype));
...
void BoxDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    if (this->output_labels_) {
      ...
      fill_truth_detection(top_label, label_offset, batch_size, flip, -dx/w, -dy/h, nw/w, nh/h);
      // LOG(INFO) << "Modified id: " << top_label[label_offset+0] << " x: " << 
      //   top_label[label_offset+1] << " y: " << top_label[label_offset+2] <<
      //    " w: " << top_label[label_offset+3] << " h: " << top_label[label_offset+4] << "\n";
