...
for x in range(image_letterbox.shape[0]):
  for y in range(image_letterbox.shape[1]):
    for k in range(image_letterbox.shape[2]):
      f.write(str(image_letterbox[x][y][k]))
      if k < image_letterbox.shape[2] - 1:
        f.write(',')
    f.write('\n')
f.close()

det(image_letterbox,'10001',image_file, label_file)
print "Done"