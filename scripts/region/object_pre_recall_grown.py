...
if len(sys.argv) != 4:
  print "Usage: python object_pre_recall_grown.py detections.csv processed_detects.csv pre_recall.csv"
  exit()
...
for idx, thresh in enumerate(threshold_range):
  tp_string = 'gtp_'+str(thresh)
  fp_string = 'gfp_'+str(thresh)
  tn_string = 'gtn_'+str(thresh)
  fn_string = 'gfn_'+str(thresh)
  ...
  results_df.loc[(results_df[grown_string]>gt_overlap) & (results_df['gt']>gt_overlap), tp_string] = 1
  results_df.loc[(results_df[grown_string]>gt_overlap) & (results_df['gt']<gt_overlap), fp_string] = 1
  results_df.loc[(results_df[grown_string]<gt_overlap) & (results_df['gt']>gt_overlap), fn_string] = 1
  results_df.loc[(results_df[grown_string]<gt_overlap) & (results_df['gt']<gt_overlap), tn_string] = 1
