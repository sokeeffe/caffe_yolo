...
if len(sys.argv) != 3:
  print "Usage: python issc2019_precall_curves.py pre_recall.csv pre_recall.csv"
  exit()
...
ax_precall = precall1_df.plot(x='recall',y='precision',
      xlim=[0.8,1.0],ylim=[0.8,1.0],title='Precision Recall Curve')
precall2_df.plot(ax=ax_precall, x='recall', y='precision',
  xlim=[0.8,1.0],ylim=[0.8,1.0],title='Precision Recall Curve')
ax_precall.set_xlabel("Recall")
ax_precall.set_ylabel("Precision")
ax_precall.legend(['Region Proposal 9', 'Region Proposal 18'], loc=3)
