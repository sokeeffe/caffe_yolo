...
if len(sys.argv) != 4:
	print "Usage: python test_microyolo_v2.py net.prototxt net.caffemodel testImage.png"
	exit()
...
def det(image, image_id, pic, im_w, im_h, net_w, net_h):
	...
	objectness = 1
	classes = 4
	biases = [0.35252,0.1977,  0.344,0.786,  1.3292,1.3388]
	...
	for i in range(output_width*output_height):
		...
		for n in range(num_boxes):
			...
			boxes[index] = Box((col + output_dim[0,box_index,row,col])/output_width,
				(row + output_dim[0,box_index+1,row,col])/output_height,
	...
	for box in boxes:
		box = correct_region_box(box, (output_width*output_height*num_boxes), im_w, im_h, net_w, net_h, 1)
	print "Box X: " + str(boxes[37].x) + " Y: " + str(boxes[37].y) + " W: " + str(boxes[37].width) + " H: " + str(boxes[37].height)
...
det(image_letterbox2,'10001',pic, image2.shape[2], image2.shape[1], 64, 64)
print 'over'
