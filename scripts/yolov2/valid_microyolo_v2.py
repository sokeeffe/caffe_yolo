...
if len(sys.argv) != 4:
  print "Usage: python valid_microyolo_v2.py valid_images net.prototxt net.caffemodel"
  exit()
...
def det(image,id, im_w, im_h, net_w, net_h):
  ...
  for indx, box in enumerate(boxes):
    box = correct_region_box(box, (output_width*output_height*num_boxes), im_w, im_h, net_w, net_h, 0)
...
for image in images:
  ...
  im = cv2_to_numpy(im)
  im_letterbox = letterbox_image(im, 64, 64)
  det(im_letterbox, os.path.basename(image_file).split('.')[0], im.shape[2], im.shape[1], 64, 64)
  # break
f_ball.close()
f_robot.close()
f_goal.close()
