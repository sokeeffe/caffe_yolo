...
if len(sys.argv) != 5:
	print "Usage: ./meanAveragePrecision.py detections.txt groundTruth.txt class outfile"
	exit()
...
results_df['positive_detection'] = 0
results_df = results_df.sort_values(by='Prob',ascending=[0])
# print results_df

###### CHANGE INPUT DIRECTORY PATH FOR GROUND TRUTH HERE
gt_df = pd.read_csv(sys.argv[2])
# print gt_df
gt_df = gt_df.loc[gt_df['Class']==int(sys.argv[3])]
...
for index, row in results_df.iterrows():
	...
	for index2, row2 in gt_df.loc[gt_df['Image']==image].iterrows():
		...
		# print iou
		if iou > IOU_thresh and gt_df.loc[index2,'detected'] != 1:
			gt_df.loc[index2,'detected'] = 1
			results_df.loc[index,'positive_detection'] = 1
...
# print results_df.head(results_df.shape[0])
# print range(1,results_df.shape[0]+1)

for idx in range(1,results_df.shape[0]+1):
	temp_results_df = results_df.head(idx)
	true_positives = temp_results_df.loc[temp_results_df['positive_detection']==1].shape[0]
	total_positives = temp_results_df.shape[0]
	total_objects = gt_df.shape[0]
...
pr_df.to_csv(sys.argv[4],index=False)
